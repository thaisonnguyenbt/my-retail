 ...
 */
package my.retail.core.components.impl;
...
@Properties(value={
        ...
        @Property(name = "sling.servlet.selectors", value = { MyRetailConstants.DELETE_CARTENTRY_SELECTOR,
                MyRetailConstants.MODIFY_CARTENTRY_SELECTOR, MyRetailConstants.ADD_CARTENTRY_SELECTOR }),
...
public class CartEntryServlet extends SlingAllMethodsServlet {
    ...
    private static final String CONTENT_MY_RETAIL_DEFAULT = "/content/my-retail/us/en/";
    ...
    protected void doPost(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException, IOException {
        ...
        if (MyRetailConstants.DELETE_CARTENTRY_SELECTOR.equals(selectorString)) {
            doDeleteProduct(request, response, session);
        } else if (MyRetailConstants.MODIFY_CARTENTRY_SELECTOR.equals(selectorString)) {
            doModifyProduct(request, response, session);
        } else if (MyRetailConstants.ADD_CARTENTRY_SELECTOR.equals(selectorString)) {
        ...
        if (AuthUtil.isAjaxRequest(request)) {
            ...
            try (JsonGenerator jsonGenerator = Json.createGenerator(response.getWriter())) {
                ...
                LOGGER.error("CartEntryServlet, {}", e);
                response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            }
        } else {
    ...
    private String renderContent(SlingHttpServletRequest request, SlingHttpServletResponse response, String contentPath)
        ...
        Page root = MyRetailHelper.findRoot(currentPage);

        String path = CONTENT_MY_RETAIL_DEFAULT + contentPath; // Fallback if root is not found
        if (root != null) {
